...
- Fixes the _Commit Graph_'s AI _Compose_ mode failing with `Hunk index N not found` when applying a plan with excluded files (and mis-mapping hunks in the plan preview), reporting spurious stash conflicts that left a stale `gitlens-compose-` stash behind, and silently discarding excluded untracked files &mdash; hunks are now indexed consistently between the plan and apply, and applying is a pure ref move with no stash round-trip, so excluded and untracked changes always remain intact in the working tree and `git status` &mdash; including the staged/unstaged split &mdash; is preserved
- Fixes the _Commit Composer_ reporting spurious stash conflicts (and leaving a stale `gitlens-compose-` stash behind) when committing or recomposing with changes left behind &mdash; e.g. unstaged changes excluded from the composition or a deselected draft commit &mdash; applying now moves the branch onto the new commits without stashing, leaving the working tree untouched
- Fixes the _Commit Composer_ failing to commit changes that include binary files &mdash; binary changes are now carried as full binary patches and committed byte-identically
- Fixes the _Commit Composer_ failing with _Unstaged changes have been modified since composer opened_ and committing nothing when untracked files are included in the composition
- Fixes the _Commit Composer_ failing to commit anything (`<old path> does not exist in index`) when a renamed-and-edited file's changes are split across multiple commits &mdash; only the first commit performs the rename; later commits apply as modifications of the renamed path
